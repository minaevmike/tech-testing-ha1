...
class NotificationPusherTestCase(unittest.TestCase):
    ...
    def test_notification_worker_ack_bury(self):
        ...
        with mock.patch('requests.post', mock.Mock(side_effect = requests.RequestException('EBury'))), mock.patch('json.dumps', mock.Mock()):
            notification_worker(task, task_queue)
    ...
    def test_mainloop_bad_config(self):
        config = make_conf()
        config.QUEUE_HOST = ''
    ...
    def test_mainloop_task_succ(self):
        config = make_conf()

        worker = mock.MagicMock()
        with mock.patch('notification_pusher.Greenlet', mock.Mock()):
            with mock.patch('tarantool_queue.tarantool_queue.Tube.take', mock.Mock(return_value = mock.Mock())):
                main_loop(config)
